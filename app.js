...
async function handleSubmit(event) {
  ...
  try {
    ...
    if (!fulfilled.length) {
      throw new Error(
        rejected[0]?.reason?.message || 'Não foi possível carregar os dados da brapi.'
      );
    }

    renderResults(fulfilled, rejected.map((item) => item.reason));
  } catch (error) {
    console.error(error);
...
async function fetchAssetData(asset, token) {
  const url = new URL(`https://brapi.dev/api/quote/${encodeURIComponent(asset.ticker)}`);
  ...
  if (!response.ok) {
    throw new Error(
      `Erro ao consultar ${asset.ticker}: ${response.status} ${response.statusText}.` +
        (response.status === 403
  ...
  if (!payload || !Array.isArray(payload.results) || !payload.results.length) {
    throw new Error(`A brapi não retornou dados para ${asset.ticker}.`);
  }
  ...
  const metrics = buildMetrics(asset.type, quote, monthlyReturns);
...
function buildMetrics(type, quote, monthlyReturns) {
  const baseMetrics = {
    ...
    volatility: computeStdDeviation(monthlyReturns.map((item) => item.value)),
  ...
  const criptoMetrics = {
    ...
    volatilidade: computeStdDeviation(monthlyReturns.map((item) => item.value)),
...
function computeStdDeviation(values) {
  ...
  const variance =
    filtered.reduce((acc, value) => acc + Math.pow(value - mean, 2), 0) / filtered.length;
